document.addEventListener('DOMContentLoaded', () => {
  ...
  Navigation.renderShell(CONSTANTS.ROLES.ADMINISTRATOR, 'notifications');
...
async function loadNotifications() {
  ...
  try {
    ...
    if (!list || list.length === 0) {
      container.innerHTML = `
        <div class="empty-state py-8">
          <div class="empty-title">No System Notifications</div>
          <div class="empty-text text-sm">You have no unread administrative notifications.</div>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = list.map(item => `
      <div class="p-3 border rounded flex items-start gap-3 ${item.isRead ? '' : 'bg-primary-light'}">
